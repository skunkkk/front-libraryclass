...
export default function VisualizarLivroUnico({ route }) {
  ...
  const id_user = decodificado?.sub;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const livroResponse = await axios.get(`http://localhost:8000/livros/${id}`);
        setLivro(livroResponse.data);

        const resenhaResponse = await axios.get(`http://localhost:8000/resenhasLivro/${livroResponse.data.isbn_livros}`);
        setResenhaLivro(resenhaResponse.data);
      } catch (error) {
        console.error(error);
      }
    };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <p style={{ color: 'black' }}>Sinopse: {livro.sinopse_livros}</p>
          <p style={{ marginTop: "10px", fontWeight: "bold", color: 'black' }}>
            Autor: {livro && livro.autor ? livro.autor.autor_nome : 'Autor Desconhecido'}
          </p>
        </div>
        <div
          style={{
